...
model = Sequential()
#---Building a Fully connected Single layer Perceptron-----------##
model.add(Dense(256, input_dim = 256, activation='relu'))
#model.add(Dense(256, input_dim = 256, activation='sigmoid'))

model.compile(loss='binary_crossentropy', optimizer='adam')    # Since we are dealing with binary[0,1] values, loss function used was binary crossentropy.
...
model.fit(X, y, epochs=400, batch_size=3) 
...
plt.title("Graph of Ffa,Fh vs Noise Standard Deviation for noise-corrupted Alphanumeric Imagery (16x16 px) for Autoassociative Single-Layer Perceptron")
